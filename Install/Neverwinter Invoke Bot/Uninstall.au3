...
If _Singleton($Name & " Uninstaller" & "Jp4g9QRntjYP", 1) = 0 Then Exit MsgBox($MB_ICONWARNING + $MB_TOPMOST, $Title, Localize("UninstallerAlreadyRunning"))
...
For $i = 1 To $delete[0]
    If $delete[$i] <> "Uninstall.exe" And $delete[$i] <> "Localization.ini" And Not FileDelete($delete[$i]) Then Exit MsgBox($MB_ICONWARNING + $MB_TOPMOST, $Title, Localize("FailedToDeleteFile", "<FILE>", $delete[$i]))
Next
...
_SelfDelete(5, 1, 1)
If @error = 1 Then Exit MsgBox($MB_ICONWARNING + $MB_TOPMOST, "_SelfDelete()", "The script must be a compiled exe to work correctly!")
If @error = 2 Then Exit MsgBox($MB_ICONWARNING + $MB_TOPMOST, "_SelfDelete()", "Unable to create temporary deletion script file!")